...
- [x] Source nicer, real review quotes from Booking.com to replace the 3 illustrative placeholders in `content.ts`'s `reviews` array — user pasted 6 real guest reviews from Booking.com (Serbian/English originals with reviewer names, country, and a couple of real avatar photo URLs — note the avatar URLs happen to be on Google's `lh3.googleusercontent.com` CDN, which doesn't mean the reviews themselves are Google reviews; corrected here after initially mislabeling them as Google-sourced). Added a `photo?: string` field to the `Review` type; Serbian-native reviews got faithful English translations and vice versa (not fabricated — translated from the real quotes), keeping first-name-only attribution matching the site's existing privacy convention. Avatars are hotlinked directly rather than downloaded/rehosted, avoiding any re-hosting/consent gray area around third-party reviewer profile photos
